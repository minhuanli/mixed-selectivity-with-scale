...
class mix_forward(object):
    ...
    def dynloop_rcsn(self,sample_list,mix_layer_data,modality_index=0,sample_index=[], f=0.5):
        ...
        for i in range(sample_list[modality_index]):
            ...
            if modality_index == Nm-1:
                ...
                for j in range(p):
                    ...
                    ###-----feed the input vector forward with the connection matrix self.J_j and sparsity f----#
                    part_cort = self.fix_sparsity( np.matmul(self.__dict__[f'J_{j}'],input_temp),f)
            ...
            #---------if not the deepest loop, then do one more recursion-----#
                ...
                self.dynloop_rcsn(sample_list,mix_layer_data,modality_index+1,sample_index,f=f)
    ...
    def order_m(self,m,f=0.5,initial_data=False):
        ...
        self.dynloop_rcsn(sample_list,mix_layer_data,modality_index=0,sample_index=[],f=f)
